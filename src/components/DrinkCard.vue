<template> <!-- Kort för bild och lite information om drink -->
    <div class="col s12 m4 xl3 drink_card_column">
        ...
        <router-link :to="'/drinks/'+drink.idDrink" tag="div" class="card grey lighten-5 z-depth-0" :class="{'wiggle': !sortDisabled}">
            <div class="card-image"><img :src="drink.strDrinkThumb" /></div>
            <div class="card-content">
                <div class="row">
                    <div class="col s12 truncate grey-text">{{drink.strCategory}}
                        <i v-if="isSaved && markFavorite" class="material-icons right yellow-text text-darken-1">star</i>
                    </div>
...
export default {
    name: "DrinkCard",
    props: ['drink', 'sortDisabled', 'markFavorite'] /* Data som skickas från förälder-komponenter */,
    computed:{
...
.wiggle{
    animation:wiggle .15s infinite ease;
}
